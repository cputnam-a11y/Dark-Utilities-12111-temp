...
public class Filters {
    ...
    public static final Predicate<Entity> WATER = e -> e instanceof WaterAnimal || (e instanceof LivingEntity living && living.getType().is(EntityTypeTags.AQUATIC)) || e instanceof Drowned || e instanceof Frog;
    ...
    private static Predicate<Entity> tagFilter(TagKey<@NotNull EntityType<?>> tag) {
    ...
    private static Predicate<Entity> hasEquipment() {
        return e -> {
            ...
            for (ItemStack armor : (Iterable<? extends ItemStack>) Arrays.stream(EquipmentSlot.values()).map(living::getItemBySlot)::iterator) {
                if (!armor.isEmpty()) {
                    return true;
                }
            }
            for (ItemStack held : (Iterable<? extends ItemStack>) Arrays.stream(InteractionHand.values()).map(living::getItemInHand)::iterator) {
                if (!held.isEmpty()) {
                    return true;
                }
            }
