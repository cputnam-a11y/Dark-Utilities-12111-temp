...
public class FlatTileEffects {
    ...
    public static final BlockFlatTile.CollisionEffect PUSH_WEAK = velocityEffect(0.06d);
    public static final BlockFlatTile.CollisionEffect PUSH_NORMAL = velocityEffect(0.3d);
    public static final BlockFlatTile.CollisionEffect PUSH_STRONG = velocityEffect(1.5d);
    public static final BlockFlatTile.CollisionEffect PUSH_ULTRA = velocityEffect(7.5d);
    ...
    private static BlockFlatTile.CollisionEffect statusEffect(Holder<@NotNull MobEffect> effect, int duration, int amplifier) {
    ...
    private static BlockFlatTile.CollisionEffect velocityEffect(double velocity) {
        return (state, world, pos, entity) -> {
            if (!entity.isCrouching()) {
                final Direction direction = state.getValue(BlockStateProperties.HORIZONTAL_FACING);
                entity.setDeltaMovement(entity.getDeltaMovement().add(velocity * (direction.getStepX() * 1.5), 0, velocity * (direction.getStepZ() * 1.5)));
            }
        };
    }

    private static DamageSource getSource(Level level, ResourceKey<@NotNull DamageType> id) {
        final Registry<@NotNull DamageType> registry = level.registryAccess().lookupOrThrow(Registries.DAMAGE_TYPE);
        final Holder.Reference<@NotNull DamageType> damage = registry.getOrThrow(id);
